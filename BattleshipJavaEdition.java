...
public class BattleshipJavaEdition {
   ...
   *
   * @param arrayForMenu String[]
   * @return menuOutput String
   */
  public static String displayMenu(String[] arrayForMenu) {
    ...
    for (String forMenu : arrayForMenu) {
      menuOutput += forMenu + "\n";
    }
   ...
   *
   * @param GRID_SIZE        final int
   * @param VACANT_SPOT_CHAR final char
   * @return newGrid char[][]
   */
  public static char[][] createGrid(final int GRID_SIZE, final char VACANT_SPOT_CHAR) {
    ...
    for (int i = 0; i < GRID_SIZE; i++) {
      ...
      Arrays.fill(newGrid[i], VACANT_SPOT_CHAR);
    }
  ...
  public static void outputGrid(char[][] gridToOutput) {
    ...
    for (int i = 0; i < gridToOutput.length; i++) {
      ...
      for (int j = 0; j < gridToOutput[i].length; j++) {
        System.out.print(gridToOutput[i][j] + "   ");
      }
   ...
   *
   * @param MESSAGE_SUFFIX final String
   * @param inputObject    Scanner
   */
   ...
   *
   * @param VACANT_SLOT   final char
   * @param OCCUPIED_SPOT final char
   * @param MAX_BOAT_SIZE final int
   * @param inputObject   Scanner
   * @param gridForPlayer char[][]
   * @return gridForPlayer char[][]
   */
            ...
            if (currentBoatOrientation.equalsIgnoreCase("horizontal") || currentBoatOrientation.equalsIgnoreCase("h")) {
              ...
              try {
                ...
                if (gridForPlayer[boatStarting_Y][boatStarting_X + j] == OCCUPIED_SPOT) {
                  ...
                  for (int k = j; k >= 0; k--) {
                    try {
                      gridForPlayer[boatStarting_Y][boatStarting_X - k] = VACANT_SLOT;
                  ...
                  gridForPlayer[boatStarting_Y][boatStarting_X + j] = OCCUPIED_SPOT;
                }
              ...
              try {
                ...
                if (gridForPlayer[boatStarting_Y + j][boatStarting_X] == OCCUPIED_SPOT) {
                  ...
                  for (int k = j; k >= 0; k--) {
                    try {
                      gridForPlayer[boatStarting_Y - k][boatStarting_X] = VACANT_SLOT;
                    } catch (ArrayIndexOutOfBoundsException e) {
                      break;
                    }
                  }
                  ...
                  gridForPlayer[boatStarting_Y + j][boatStarting_X] = OCCUPIED_SPOT;
                }
              } catch (ArrayIndexOutOfBoundsException exception) {
                // Reverts the grid's values to prevent them from being left
                // over if the starting coordinate is valid but the rest
   ...
   * @param OCCUPIED_SPOT   final char
   * @param MAX_BOAT_SIZE   final int
   * @param randObject      Random
   * @param gridForComputer char[][]
   * @return gridForComputer char[][]
   */
  public static char[][] populateComputerGrid(final int GRID_SIZE, final char OCCUPIED_SPOT,
    ...
    for (int i = MAX_BOAT_SIZE; i >= 2; i--) {
      ...
      if (i < 5) {
        horizontalOrVertical = randObject.nextInt(2);
      }
      ...
      if (horizontalOrVertical == 1) {
        ...
        boatStarting_Y = switch (i) {
          case 3 -> 2;
          case 4 -> 5;
          default -> boatStarting_Y;
        };
      } else {
        ...
        // Carrier on a Y-value of 9
        boatStarting_Y = switch (i) {
          case 3 -> randObject.nextInt(3) + 2;
          case 4 -> randObject.nextInt(4) + 5;
          case 5 -> 9;
          default -> boatStarting_Y;
        };
      }
      ...
      for (int j = 0; j < i; j++) {
        ...
        if (horizontalOrVertical == 1) {
          gridForComputer[boatStarting_Y + j][boatStarting_X] = OCCUPIED_SPOT;
        } else {
          gridForComputer[boatStarting_Y][boatStarting_X + j] = OCCUPIED_SPOT;
        }
      }
   ...
   * @param GRID_SIZE   final int
   * @param inputObject Scanner
   * @return playerGuessedCoordinates int[]
   */
  public static int[] playerGuess(final int GRID_SIZE, Scanner inputObject) {
    ...
    while (isInvalid) {
      ...
      try {
        ...
        if (playerGuess_X >= GRID_SIZE || playerGuess_Y >= GRID_SIZE) {
          System.out.println(OUT_OF_BOUNDS_ERROR);
        } else {
          isInvalid = false;
        }
    ...
    return new int[]{playerGuess_X, playerGuess_Y}; // Returns their guess as an integer array
  }
   ...
   * @param GRID_SIZE           final int
   * @param randObject          Random
   * @param lastWasHit          boolean
   * @param previousCoordinates int[]
   * @return computerGuessedCoordinates int[]
   */
    ...
    if (lastWasHit) {
      ...
      switch (nextGuessDirection) {
        case 0: // Up
          ...
          // off the board
          if (previousCoordinates[1] - 1 < 0) {
            computerGuessedCoordinates[1] = randObject.nextInt(GRID_SIZE);
          } // Sets the value at random instead
          else {
            computerGuessedCoordinates[1] = previousCoordinates[1] - 1;
          }
        ...
        case 1: // Right
          ...
          // guess but catches this action if the new coordinate overflows
          // off the board
          if (previousCoordinates[0] + 1 >= GRID_SIZE) {
            computerGuessedCoordinates[0] = randObject.nextInt(GRID_SIZE);
          } // Sets the value at random instead
          else {
            computerGuessedCoordinates[0] = previousCoordinates[0] + 1;
          }
        ...
        case 2: // Down
          ...
          // guess but catches this action if the new coordinate overflows
          // off the board
          if (previousCoordinates[1] + 1 >= GRID_SIZE) {
            computerGuessedCoordinates[1] = randObject.nextInt(GRID_SIZE);
          } // Sets the value at random instead
          else {
            computerGuessedCoordinates[1] = previousCoordinates[1] + 1;
   ...
   *
   * @param OCCUPIED_SPOT   final char
   * @param playerGuessing  boolean
   * @param guess           int[]
   * @param gridForPlayer   char[][]
   * @param gridForComputer char[][]
   * @param gameplayGrid    char[][]
   * @return targetStatus boolean
   */
  public static boolean hitOrMiss(final char OCCUPIED_SPOT, boolean playerGuessing,
    ...
    final char HIT_SPOT = 'X';
    final char MISSED_SPOT = '|';
   ...
   *
   * @param currentTurn_hit  boolean
   * @param remainingTargets int
   * @return remainingTargets int
   */
  ...
  public static void gameSummary(boolean playerWon, int score,
    ...
    // one accordingly
    if (playerWon) {
      System.out.println("CONGRATULATIONS! You win this round of Battleship against the computer, taking you " + score + " turns to do so.");
    } else {
      System.out.println("AW MAN! The computer wins this round of Battleship.");
    }
  ...
  public static void main(String[] args) {
    ...
    final char EMPTY_SLOT_CHAR = '~';
    final char FILLED_SLOT_CHAR = 'O';
    ...
    boolean computerHitLast = false;
    int[] computerLastGuess = new int[2]; // Stores its last coordinate guess

    int playerRemainingTargets;
    int computerRemainingTargets;

    int numberOfMoves; // Stores the amount of moves the player makes
    ...
    while (!exitProgram) {
      ...
      if (userSelection.equalsIgnoreCase("a") || userSelection.equalsIgnoreCase("b")) {
        if (userSelection.equalsIgnoreCase("a")) {
          greetPlayer(scanner);
        } // Show the game's introduction and rules
        ...
        playerRemainingTargets = 14;
        computerRemainingTargets = 14;
      ...
      // point to an option listed on the program menu itself
        ...
        System.out.println("That menu option doesn't exist! Try again.");
      }
    }
